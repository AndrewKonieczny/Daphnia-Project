...
# To use this function, you need to import a .csv file of the raw ImageJ
# data. To prepare the .csv file, you need to take the individual animal
# observation periods and collect them into a single .csv file. Then,
# depending on how you concatinate the collected data you take note of
# what the interval is between individual animal observations is and put
# that value into the DataInterval position of the function. For example,
# if you have two observation periods and two animals in the same drug
# concentration, you may have recorded, using ImageJ, the ID, Area, X,
# Y, and Slice. If you recorded this information, in this order, you have
# an interval of 5 since you have 5 categories of data. Only the Area and
# X data are examined here. The ID has no meaning here and the Y
# displacements are minimal and add very little to the force measurements 
# being made. Slice only has importance when the time is needed for
# a time series analysis, which this script does not address. The output
# is an array of force measurements that can be compared at various drug
# concentrations. This script also removes the feeding oscilations (which
# is only important here because it skewes the mean force measuments).

Report <- function(InputData,DataInterval){
  ...
  # Takes in input data, and goes therough a series of possible
  # lower bounds to output an array of means.
  ...
  # data.
  ReportedData <- function(data,Interval){
    BestBound <- sum(ifelse(abs(dx(dx(lowerbounds(data,100))))>1,1,0))+1
    ForceData <- ForceFilter(concat(data,Interval),BestBound)*0.0518
    return(ForceData)
  }
  
  Aconcat <- function(Inputs){
    final <- NULL
    Max <- dim(Inputs)[2]/2
    elementSequence <- seq(1,Max,2)
    for (i in elementSequence){ 
      final <- c(final,area_dev(Inputs,i))
    }
    out <- 1-sum(final,na.rm=TRUE)/length(final)
    return(out)
  }
  out <- ReportedData(InputData,DataInterval)
  print('Percent of Points Removed Due to Camera Inaccuracies')
  print(Aconcat(InputData))
  return(out)
}
...
zero <- function(data, col){
  ...
  binned <- .bincode(v.data, breaks = bin, right = TRUE,
                     include.lowest = TRUE)
  x <- v.data[as.numeric(which.max(table((.bincode(v.data, breaks = bin, right = TRUE, include.lowest = TRUE)))))]
